<x-app-layout>
  ...
  <div class="conatainer px-10 my-10">
    <div class="bg-white p-4">
      <form method="POST" action="{{ route('projet.update',$project->id) }}" enctype="multipart/form-data">
        @csrf
        ...
        <div class="flex">
          <label for="photo" class="w-1/3 min-h-[300px] bg-gray-100 mr-2 relative after:w-full after:h-full after:bg-slate-600 after:bg-opacity-30 after:absolute after:z-10 after:opacity-0 hover:after:opacity-100 after:transition cursor-pointer">
            <img id="image" class="absolute w-full h-full object-cover" src="{{ $project->image ? asset("storage/$project->image"):asset('static/img/product.png') }}" alt="product">
            @error('image')
          ...
          <div class="w-2/3 ml-2">
            <div>
              ...
              <textarea name="description" placeholder="Entrer la description du projet ici … " class="w-full placeholder:italic rounded-md shadow-sm border-gray-300 focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50" id="" cols="30" rows="3">{{ old('description',$project->description) }}</textarea>
            </div>
            <div class="flex pb-3 ">
              ...
              <div class="mt-1 w-1/2 ml-1">
                  ...
                  <x-text-input placeholder='Combien de temps a durée le projet ?' id="duree" class="w-full placeholder:italic" type="text"
                      name="duree" required :value="old('duree',$project->duree)" />
                  @error('duree')
                      <span class="text-sm text-red-400 block">{{ $message }}</span>
                  @enderror
              </div>
            ...
            <div class="flex mt-2 pb-3 items-center">
              ...
              <div class="mt-4 w-full ml-1">
                  <label for="online" class="inline-flex cursor-pointer items-center justify-center mt-1">
                    <span>Visible en ligne ? </span> <input @checked($project->online) class="text-lg w-6 h-6 text-primary ring-0 border-2 focus:ring-0 focus:outline-none  inline-block ml-3" type="checkbox" name="online" id="online">
                  </label>
              </div>
            </div>
          </div>
        ...
        <div class="mt-2">
          <div class="my-3">
            <x-primary-button>Mettre a jour le projet</x-primary-button>
          </div>
        </div> 
    </form>
    </div>
